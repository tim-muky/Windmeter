...
Two wires from the anemometer come down through the ~7 m mast cable. Add three
small parts at the Pico end so the long line runs reliably.

```
                          ┌── 10 kΩ ──► 3V3 (pin 36)
                          │
   GP15 (pin 20) ── 1 kΩ ─●── (up 7 m of shielded cable) ──► anemo signal
                          │
                          └── 10 nF ──► GND

   GND rail ────────────────────────── (cable ground + shield) ──► anemo GND
```

**Parts (all mount near the Pico, not at the masthead):**
- **10 kΩ** resistor — pull-up to 3V3
- **1 kΩ** resistor — in series with GP15
- **10 nF** ceramic cap (marked `103`) — GP15 to GND
- 8 m of shielded 2-core cable; shield to GND **at the Pico end only**

Optional: a **3.3 V zener** or **SMBJ3.3A TVS** from GP15 to GND for lightning /
surge protection.

No firmware change — the internal pull-up stays on, the external 10 kΩ just
makes the line stronger against noise and cable capacitance.
